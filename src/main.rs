...
fn run() -> Result<(), VNextError> {
    ...
    let (current_version, _) = version::find_version_base(&repo, &head);
    if cli.current {
    ...
    // Calculate version
    let (next_version, mut summary) = match version::calculate_version(&repo, &head, &major_re, &minor_re, &noop_re, &breaking_re) {
        Ok(result) => result,
        Err(e) => {
    ...
    // Output result
    changelog::output_result(&next_version, &summary, cli.changelog, cli.no_header_scaling, &current_version, &repo_info);
